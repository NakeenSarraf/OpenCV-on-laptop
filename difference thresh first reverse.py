...
# Apply Otsu’s thresholding first
mask = sitk.OtsuThreshold(raw_img_sitk, 0, 1)

# Perform N4 bias field correction on the thresholded image
...
# Shrink the images **only in X and Y** for faster processing (preserve slices)
shrinkFactor = [1, 4, 4]  # Keep all slices, shrink X & Y by 4
...
difference_array = original_array - corrected_array
...
# Ensure we can access slice 22 (index 21)
